...
<template>
    <div class="mini app-drag">
        ...
        <div style="width:100%;height:50px;" class="clearfix">
            <div v-if="word" class="ball" :class="word && ex && url ? 'suc' : 'danger'" @click="set('word')" @mouseenter="show('word')" @mouseout="shown = ''">
                {{this.word.substr(0, 7)}}
            </div>
            <div v-else class="ball err" @click="set('word')">
                word
            </div>
            <div v-if="ex" class="ball" :class="word && ex && url ? 'suc' : 'danger'" @click="set('ex')" @mouseenter="show('ex')" @mouseout="shown = ''">
                {{this.ex.substr(0, 7)}}
            </div>
            <div v-else class="ball err" @click="set('ex')">
                example
            </div>
            <div v-if="url" class="ball" :class="word && ex && url ? 'suc' : 'danger'" @click="set('url')" @mouseenter="show('url')" @mouseout="shown = ''">
                {{this.url.substr(0, 7)}}
            </div>
        ...
        <div v-if="shown" style="width: 100%; height: 100px; text-align:center; font-weight: bold; overflow: scroll; margin-top: 20px;">
            {{shown}}
        </div>
        <div v-else style="width: 100%; height: 100px; text-align:center; font-weight: bold; overflow: scroll; margin-top: 20px;">
            {{message}}
        </div>
    </div>
</template>
